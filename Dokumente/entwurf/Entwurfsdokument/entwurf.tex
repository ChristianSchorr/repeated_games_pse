...
\subsection{edu.kit.loop.model.distribution}
...
\subsubsection{Class \texttt{CapitalEquilibrium}}
Implements: \texttt{CountingEquilibriumCriterion}
...
Konstruktoren:
\begin{itemize}\itemsep -10pt
\item \texttt{CapitalEquilibrium(double alpha, int G)}
\item[] Erzeugt ein neues \texttt{CapitalEquilibrium}-Objekt mit den gegebenen Parametern.
\item[] \texttt{alpha}: die Strenge des Gleichgewichts
\item[] \texttt{G}: die minimale Zahl aufeinanderfolgender Gleichgewichtsbedingung-erfüllender Adaptionsschritte
\end{itemize}

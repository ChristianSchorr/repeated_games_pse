    ...
    public List<Agent> initialiseAgents(EngineSegment segment, boolean mixedStrategiesAllowed) {
        ...
        if (mixedStrategiesAllowed) {
            for (int i = 0; i < strategies.size(); i++) {
                ...
                wrappedSingleStrategy.add(strategies.get(i));
                List<Double> probabilities = new ArrayList<Double>();
                probabilities.add(1.0);
                MixedStrategy mixedStrategy = new MixedStrategy("--autogenerated--", "--autogenerated--", wrappedSingleStrategy, probabilities);
                strategies.remove(i);
